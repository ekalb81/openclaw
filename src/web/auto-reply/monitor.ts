...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    try {
      ...
      if (stopRequested() || sigintStop || !keepAlive) {
        break;
      }
      ...
      if (reconnectPolicy.maxAttempts > 0 && reconnectAttempts >= reconnectPolicy.maxAttempts) {
        reconnectLogger.warn(
          ...
          "web reconnect: max attempts reached after startup failures; stopping monitor",
        );
        runtime.error(
      ...
      reconnectLogger.info(
        ...
        "web reconnect: scheduling startup retry",
      );
      runtime.error(
        `WhatsApp Web listener startup failed. Retry ${reconnectAttempts}/${reconnectPolicy.maxAttempts || "∞"} in ${formatDurationPrecise(delay)}… (${errorStr})`,
      );
      try {
